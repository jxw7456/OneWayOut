        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            switch (game.state)
            {
                ...
                case GameState.GAME:
                    ...
                    //Arrows
                    if (player.arrowSupply > 0 && arrowExist == false)
                    {
                        arrow = new ArrowC(100, arrowPicture, player.position.X + 50, player.position.Y + 25, player.position.Width, player.position.Height);
                        arrowExist = true;
                        arrow.Distance(slime, asset.slimes);

                        if (player.direction == Direction.RIGHT)
                        {
                            arrow = new ArrowC(100, arrowPicture, player.position.X + 50, player.position.Y + 25, player.position.Width + 20, player.position.Height);
                            arrowExist = true;
                            arrow.Distance(slime, asset.slimes);
                        }
